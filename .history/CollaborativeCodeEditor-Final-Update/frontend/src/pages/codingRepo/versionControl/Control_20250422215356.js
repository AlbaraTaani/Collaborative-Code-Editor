...
const Control = ({ repo, currentFile, isConnected, subscribeToCodeUpdates }) => {
    ...
    const downloadProject = async ({ branchName }) => {
        try {
            const response = await fetch(`http://localhost:8080/api/projects/${repo.repoId}/${branchName}/download-project`, {
                method: 'GET',
                headers: {
    ...
    const handleCreateNewProject = async (branchName) => {
        try {
            const response = await fetch(`http://localhost:8080/api/projects/create-project`, {
                ...
                body: JSON.stringify({ projectName: branchName, repoId: repo.repoId }),
            });
        ...
        if (filename && branchName) {
            try {
                const response = await fetch(`http://localhost:8080/api/files/create-file`, {
                    ...
                    body: JSON.stringify({ filename: filename, repoId: repo.repoId, projectName: branchName, extension: extension }),
                });
    ...
    return (
        <div className="version-control">
            ...
            <div className="version-control-buttons">
                <button className="action-btn add-branch-btn" onClick={() => setShowProjectModal(true)}>
                    Add New Project
                </button>
            ...
            <Action
                ...
                actionLabel="Create"
                inputs={[{ label: 'Project Name', placeholder: 'Enter new branch name', name: 'branchName' }]}
                onConfirm={(inputValues) => {
                    handleCreateNewProject(inputValues.branchName);
            ...
            <Action
                ...
                actionLabel="Create"
                inputs={[
                    { label: 'Project Name', placeholder: '/ Enter branch name', name: 'branchName' },
