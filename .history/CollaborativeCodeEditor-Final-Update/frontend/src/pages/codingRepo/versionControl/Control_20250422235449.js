...
const Control = ({
  ...
  const downloadProject = async ({ projectName }) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/projects/${repo.repoId}/${projectName}/download-project`,
        {
          method: "GET",
          headers: {
      ...
      showError(
        "You do not have projects currently. Please add a new project."
      );
    }
  };
  ...
  const fetchProjects = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/projects/repo/${repo.repoId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const fetchFiles = async (projectName) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/files/list-files/${encodedProjectName}/${repo.repoId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const fetchFileContent = async (file) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/files/pull-file-content`,
        {
          method: "POST",
          headers: {
